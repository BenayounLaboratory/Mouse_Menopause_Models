...
# Load Seurat object
load("~/Dropbox/Benayoun_lab/Projects/Menopause_model_project/Data/For_Github/4_Ovarian_scRNAseq/8_PB_TE/Input/2025-01-09_10x_ovary_Benayoun_lab_Foxl2_NULL_Seurat_object_scTE_combined_FINAL.RData")

DefaultAssay(ovary.Foxl2.genes.TEs.combined.seurat) <- "RNA"

# Extract metadata

# Subset only the relevant columns for pseudobulk analysis
metadata <- ovary.Foxl2.genes.TEs.combined.seurat@meta.data %>%
...
sce <- as.SingleCellExperiment(ovary.Foxl2.genes.TEs.combined.seurat)

rm(ovary.Foxl2.genes.TEs.combined.seurat)
...
for (cell_type in names(counts.pb)) {
  ...
  my.outprefix <- paste0(Sys.Date(), "_Foxl2_PB_with_scTE_DEseq2_", cell_type)
...
my.outprefix <- paste0(Sys.Date(), "_Foxl2_PB_with_scTE_DEseq2_B_")
...
# Save
save(vst.counts, file = file.path(output_dirs$results, paste0(Sys.Date(), "_Foxl2_PB_with_scTE_VST_counts.RData")))
save(deseq.res.list, file = file.path(output_dirs$results, paste0(Sys.Date(), "_Foxl2_PB_with_scTE_DESeq2_object.RData")))
...
get_TE_GSEA_bubble_plot <- function(gsea_summary_df) {
  ...
  pdf_filename <- paste(Sys.Date(), "Foxl2_GSEA_TE_BUBBLE_plot.pdf", sep = "_")
...
################################################################################
# Save session info
sink(file = paste0(Sys.Date(), "_Foxl2_TE_analysis_Session_Info.txt"))
sessionInfo()
sink()
