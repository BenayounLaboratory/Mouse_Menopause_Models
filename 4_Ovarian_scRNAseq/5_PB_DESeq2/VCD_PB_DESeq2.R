...
# Load Seurat object
load("/Volumes/jinho01/Benayoun_lab/Projects/Menopause_model_project/Data/Ovarian_scRNAseq/0_Annotated_Seurat_objects/Without_scTE/2024-10-24_10x_ovary_Benayoun_lab_VCD_Seurat_object_with_final_annotation.RData")
...
for (cell_type in names(counts.pb)) {
  ...
  my.outprefix <- paste0(Sys.Date(), "_VCD_PB_DEseq2_", cell_type)
...
# Save
save(vst.counts, file = file.path(output_dirs$results, paste0(Sys.Date(), "_VCD_PB_VST_counts.RData")))
save(deseq.res.list, file = file.path(output_dirs$results, paste0(Sys.Date(), "_VCD_PB_DESeq2_object.RData")))
...
# Filter and reorder the results
...
pdf(paste0(Sys.Date(), "_VCD_stripplot_CTL_vs_VCD_significant_genes.pdf"), width = 6, height = 5)
...
plot(
  ...
  ylim = c(-15, 20),
  axes = FALSE,
  xlab = "", ylab = "Log2 fold change (VCD / CTL)"
)
...
for (i in seq_along(deseq.res.list)) {
  ...
  colors[sig_genes & current_result$log2FoldChange > 0] <- "yellow"     # Up in VCD
  colors[sig_genes & current_result$log2FoldChange < 0] <- "deeppink1"  # Up in CTL
...
################################################################################
# Save session info
sink(file = paste0(Sys.Date(), "_VCD_PB_DESeq2_analysis_Session_Info.txt"))
sessionInfo()
sink()
